...
export class MapContainer extends React.Component<{ Sites: Site[], SetLayout: any, SetSelectedSite: any, SelectedSite: Site, layoutupdate: boolean }, { Map: any }> {
    ...
    private _renderMap = (() => {
        ...
        this.Map = withGoogleMap((props: { sites: Site[], markers: MarkerWrappers, SelectedSite: Site }) => {
            return (
                <GoogleMap defaultZoom={8} center={{ lat: Number(props.SelectedSite.latitude), lng: Number(props.SelectedSite.longitude) }}>
                    <MarkerClusterer
                        averageCenter
                        enableRetinaIcons
                        gridSize={60}
        ...
        return function (sites: Site[], ID: number, SetSelectedSite: any, SelectedSite: Site, layout: boolean) {
            ...
            let map_ele: any = <Map_new sites={sites} SelectedSite={SelectedSite} containerElement={container} mapElement={map} markers={<MarkerWrappers Sites={sites} ClickedId={ID} SetSelectedSite={SetSelectedSite} />} />
